...
- [ ] [Review][Decision] Passive tree node icons not researched — Epic 2's goal is "icon-accurate hexagonal nodes" on all trees (passive + mastery), but the spike only researched skill icons. `skill_icons_assets_all.bundle` likely does not contain passive node icons. Options: (a) extend spike scope to locate the passive node icon bundle, (b) defer to a separate spike task, (c) explicitly document as out of scope for Story 2.1 and add a note to Story 2.4 to investigate.
